  ...
  methods: {
    ...
    addEntity(span, role) {
      ...
      this.openieEntities.push([
        ...
        [
          [
            this.tokens[span[0]].characterOffsetBegin,
            this.tokens[span[1] - 1].characterOffsetEnd,
          ],
        ],
      ]);
    },
    ...
    render(data) {
      ...
      for (this.sentI = 0; this.sentI < data.sentences.length; ++this.sentI) {
        ...
        if (
          ...
          typeof this.tokens[0].pos != "undefined"
        ) {
        ...
        if (typeof deps != "undefined") {
          this.depsRelations = this.depsRelations.concat(
            this.processDeps("dep", deps)
          );
        }
        if (typeof deps2 != "undefined") {
          this.deps2Relations = this.deps2Relations.concat(
            this.processDeps("dep2", deps2)
          );
        }
        ...
        if (
          this.tokens.length > 0 &&
          typeof this.tokens[0].lemma != "undefined"
        ) {
          for (let i = 0; i < this.tokens.length; i++) {
            let token = this.tokens[i];
            ...
            this.lemmaEntities.push([
              "LEMMA_" + this.sentI + "_" + i,
              lemma,
              [[begin2, end]],
            ]);
          }
        ...
        if (
          this.tokens.length > 0 &&
          typeof this.tokens[0].ner != "undefined"
        ) {
          for (let i = 0; i < this.tokens.length; i++) {
            var ner = this.tokens[i].ner;
            if (ner == "O") continue;
            let j = i;
            while (j < this.tokens.length - 1 && this.tokens[j + 1].ner == ner)
              j++;
            ...
            this.nerEntities.push([
              ...
              [
                [
                  this.tokens[i].characterOffsetBegin,
                  this.tokens[j].characterOffsetEnd,
                ],
              ],
            ]);
            i = j;
        ...
        if (typeof sentence.sentiment != "undefined") {
          ...
          this.sentimentEntities.push([
            ...
            [
              [
                this.tokens[0].characterOffsetBegin,
                this.tokens[this.tokens.length - 1].characterOffsetEnd,
              ],
            ],
          ]);
        }
        ...
        if (this.tokens.length > 0) {
          for (let i = 0; i < this.tokens.length; i++) {
            ...
            let j = i;
            while (
              j < this.tokens.length - 1 &&
              this.tokens[j + 1].entitylink == link
            )
              j++;
        ...
        if (typeof sentence.openie != "undefined") {
          ...
          for (let i = 0; i < sentence.openie.length; ++i) {
            ...
            if (
              parseInt(relationSpan[0]) < 0 ||
              parseInt(relationSpan[1]) < 0
            ) {
